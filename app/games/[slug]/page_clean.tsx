  ...
  useEffect(() => {
    async function fetchGameData() {
      try {
        ...
        const linksResponse = await fetch(`/api/games/${gameData.id}/download-links`)
        if (linksResponse.ok) {
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    {game.genre && game.genre.map((genre: string) => (
                      <span 
                        key={genre} 
          ...
          <section className="glass-card p-6 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {game.screenshots && game.screenshots.map((screenshot: string, index: number) => (
                ...
                <div className="space-y-6">
                  ...
                  {game.system_requirements.minimum && (
                    <div>
                      ...
                      <ul className="space-y-3">
                        {Object.entries(game.system_requirements.minimum).map(([key, value]: [string, any]) => (
                  ...
                  {game.system_requirements.recommended && Object.keys(game.system_requirements.recommended).length > 0 && (
                    <div>
                      ...
                      <ul className="space-y-3">
                        {Object.entries(game.system_requirements.recommended).map(([key, value]: [string, any]) => (
                          <li key={key} className="flex items-start gap-3">
                            <div className="mt-0.5">
                              {key === 'os' && <Monitor className="w-4 h-4 text-game-accent" />}
                              {key === 'processor' && <Cpu className="w-4 h-4 text-game-accent" />}
