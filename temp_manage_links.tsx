...
export default function ManageLinksPage({ params }: { params: { id: string } }) {
  ...
  const handleAddLink = async () => {
    ...
    try {
      ...
      const addedLink = await addDownloadLink(gameId, {
        ...
        size_bytes: newLink.size_bytes || 0,
        part: newLink.part || 1,
        total_parts: newLink.total_parts || 1
      })
  ...
  const startEditing = (link: DownloadLink) => {
    ...
    setNewLink({
      ...
      url: link.url,
      type: link.type,
      size_bytes: link.size_bytes || 0,
      part: link.part || 1,
      total_parts: link.total_parts || 1,
      game_id: gameId
    })
  }
          ...
          {showAddForm && (
            <div className="bg-game-darker rounded-lg p-4 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Dimensiune (bytes)</label>
                  <input
                    type="number"
                    name="size_bytes"
                    value={newLink.size_bytes}
                    onChange={handleNewLinkChange}
                    placeholder="ex: 10737418240 (pentru 10 GB)"
                    className="w-full bg-game-gray border border-gray-700 rounded px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-game-accent"
                  />
                </div>
                
                <div>
                  <label className="block text-sm font-medium mb-1">Parte</label>
                  <input
                    type="number"
                    name="part"
                    value={newLink.part}
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Total Părți</label>
                  <input
                    type="number"
                    name="total_parts"
                    value={newLink.total_parts}
                    onChange={handleNewLinkChange}
                    min="1"
                    className="w-full bg-game-gray border border-gray-700 rounded px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-game-accent"
                  />
              ...
              <table className="w-full text-white">
                <thead className="bg-game-darker text-left">
                  <tr>
                    ...
                    <th className="py-3 px-4">Partea</th>
                        ...
                        {editingLinkId === link.id.toString() ? (
                          <div className="flex space-x-2">
                            <input
                              ...
                              onChange={handleNewLinkChange}
                              min="1"
                              className="w-16 bg-game-gray border border-gray-700 rounded px-2 py-1 text-white focus:outline-none focus:ring-2 focus:ring-game-accent"
                            />
                          ...
                          <div className="text-gray-300 text-sm">
                            Partea {link.part || 1} din {link.total_parts || 1}
                          </div>
                        )}
                      </td>
                      <td className="py-3 px-4">
